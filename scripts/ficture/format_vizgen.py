...
def format_vizgen():
    """Format Vizgen MERSCOPE transcripts into FICTURE input."""
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--input",
        type=str,
        help="Input transcript file from Vizgen MERSCOPE, likely named like detected_transcripts.csv.gz",
    )
    ...
    parser.add_argument(
        "--coor_minmax", type=str, default="", help="Record coordinate ranges to a file"
    )
    parser.add_argument(
        "--dummy_genes",
        type=str,
        default="",
        help="A single name or a regex describing the names of negative control probes",
    )
    parser.add_argument(
        "--precision",
        type=int,
        default=2,
        help="Number of digits to store the transcript coordinates in micrometer",
    )
    ...
    for chunk in pd.read_csv(args.input, header=0, chunksize=500000, index_col=0):
        if args.dummy_genes != "":
            chunk = chunk[
                ~chunk.gene.str.contains(
                    args.dummy_genes, flags=re.IGNORECASE, regex=True
                )
            ]
    ...
    if os.path.exists(os.path.dirname(args.feature)):
        avail_columns = list(set(["gene"]).intersection(set(feature.columns)))
        #        avail_columns = list(set(['gene','transcript_id']).intersect(set(feature.columns)))
        feature = feature.groupby(by=avail_columns).agg({"Count": sum}).reset_index()
