...
def compute_f1_samples(args, base_path):
    """Compute F1 score comparing ficture and annotation."""
    ...
    if args.correct_celltypes:
        ...
        for i in factor_to_celltype.keys():
            correct_celltypes[factor_to_celltype[i]] = true_cluster[
                factor_to_celltype[i]
            ]
    else:
...
if __name__ == "main":
    parser = argparse.ArgumentParser(
        description="Compute F1 statistics based on Ficture."
    )
    parser.add_argument("method", help="Method name.")
    parser.add_argument("cohort", help="Cohort name.")
    parser.add_argument(
        "data", choices=["area", "variance", "means"], help="Data to use."
    )
    parser.add_argument(
        "--correct_celltypes",
        action="store_true",
    ...
    compute_f1_samples(args, base_path=base_path)
