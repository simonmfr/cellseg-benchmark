...
def main(data_path, sample, base_segmentation):
    ...
    sdata = read_zarr(join(path, base_segmentation, "sdata.zarr")) #enthält keine Bilder oder transcripte
...
# backing für memory efficiency
    sdata.write(join(path, f"Proseg_{base_segmentation}", "sdata_tmp.zarr"), overwrite=True)
