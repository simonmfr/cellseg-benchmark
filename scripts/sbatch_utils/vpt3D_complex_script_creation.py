...
experiment_json_path = str(Path(__file__).parents[2] / "configs" / f"vpt_3D_{args.staining1}_{args.staining2}.json")

Path(
    "/dss/dssfs03/pn52re/pn52re-dss-0001/cellseg-benchmark/misc/sbatches/sbatch_vpt_3D_complex"
).mkdir(parents=False, exist_ok=True)
for key, value in data.items():
    res_path = f"/dss/dssfs03/pn52re/pn52re-dss-0001/cellseg-benchmark/samples/{key}/results/vpt_3D_DAPI_{args.staining1}_{args.staining2}"
    vzg_path = None
    for dire in listdir(value["path"]):
        if dire.endswith(".vzg") or dire.endswith(".vzg2"):
            vzg_path = join(value["path"], dire)
    f = open(
        f"/dss/dssfs03/pn52re/pn52re-dss-0001/cellseg-benchmark/misc/sbatches/sbatch_vpt_3D_complex/{key}_{args.staining1}_{args.staining2}.sbatch",
        "w",
    )
    f.write(f"""#!/bin/bash
