...
"$CONDA_PREFIX/bin/time" -v \
  -o "/dss/dssfs03/pn52re/pn52re-dss-0001/cellseg-benchmark/misc/logs/outputs/Proseg_3D_{key}_CP1_{args.staining}_vxl_{args.voxel}_$(date +%Y%m%d_%H%M%S).time" \
python ~/gitrepos/cellseg-benchmark/scripts/segmentation/proseg_3D.py {value["path"]} {key} Cellpose_1_{args.staining}_model \
...
"$CONDA_PREFIX/bin/time" -v \
  -o "/dss/dssfs03/pn52re/pn52re-dss-0001/cellseg-benchmark/misc/logs/outputs/Proseg_3D_{key}_CP{args.CP_version}_{args.staining}_vxl_{args.voxel}_$(date +%Y%m%d_%H%M%S).time" \
python ~/gitrepos/cellseg-benchmark/scripts/segmentation/proseg_3D.py {value["path"]} {key} Cellpose_{args.CP_version}_DAPI_{args.staining} \
--voxel-layers {args.voxel} --output-cell-polygon-layers cell-polygons-layers.geojson.gz 
""")
        f.close()
