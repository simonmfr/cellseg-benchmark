...
for key, value in data.items():
    with open(
        BASE_PATH / f"misc/sbatches/sbatch_{METHOD}_to_sdata/{key}.sbatch", "w"
    ) as f:
        f.write(f"""#!/bin/bash
...
python "$HOME/gitrepos/cellseg-benchmark/scripts/seg_postprocessing/baysor_denovo_to_sdata.py" \\
  "{value["path"]}" \\
