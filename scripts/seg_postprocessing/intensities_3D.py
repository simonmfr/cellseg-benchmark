...
def main():
    ...
    parser.add_argument(
        "--boundary_key", type=str, default=None, help="Key of 3D boundary in sdata."
    )
    ...
    if args.boundary_key is not None:
        ...
        if args.method.startswith("vpt_3D"):
            ...
            if "boundaries_vpt_3D" in sdata.shapes.keys():
                ...
                logger.debug(
                    "Loading boundaries for Watershed_Merlin by path. "
    ...
    for idx in boundaries["ZIndex"].unique():
        ...
        sd.transformations.set_transformation(
            sdata_working[f"boundaries_z{idx}"], transform
        )
    ...
    for idx in [x.split("_")[-1] for x in sdata_working.shapes.keys()]:
        ...
        sdata_working["table"].obs[
            sdata_working["table"].uns["spatialdata_attrs"]["region_key"]
        ] = f"boundaries_{idx}"
        sdata_working["table"].obs[
            sdata_working["table"].uns["spatialdata_attrs"]["region_key"]
        ] = (
        ...
        logger.debug(
            sd.transformations.get_transformation(sdata_working[f"boundaries_{idx}"])
        )
        logger.debug(
    ...
    logger.info(
        f"Write out csv with updated intensities to {sdata_path / 'Intensities_3D'}."
    )
