...
def main():
    parser = argparse.ArgumentParser(
        description=("wrapper for intensities_3D to apply sample-wise.")
    )
    ...
    parser.add_argument(
        "--method_names",
        action="store_true",
        help="Method names are provided instead of segmentation algorithms.",
    )
    parser.add_argument(
        "--method",
        nargs=argparse.REMAINDER,
        type=str,
        help="If --method_names then provide a list of methods with flavors, e.g. Proseg_3D_Cellpose_1_nuclei_model. Otherwise provide list of segmentation algorithms, e.g. Proseg_3D. If not provided all intensities of available 3D segmentation methods will be computed.",
    )
    ...
    if args.method_names:
        ...
        for method_name in args.method:
            ...
            if not os.path.exists(sample_path / "results" / method_name / "sdata.zarr"):
                logger.error(
                    f"Method {method_name} does not exist or doesn't contain a sdata.zarr file."
                )
            else:
        ...
        if args.method is None:
            ...
            for method_name in os.listdir(sample_path / "results"):
                if os.path.exists(
                    sample_path / "results" / method_name / "sdata.zarr"
                ) and any([method_name.startswith(x) for x in args.method]):
                    logger.info(f"Method {method_name} identified for computation.")
                    methods.append(method_name)
    ...
    for method in methods:
        cmd = [
            sys.executable,
            str(intensities_3D_script),
            str(sample_path / "results" / method),
            args.data_path,
            "--method",
            method,
        ]
        ...
        if exit_code.returncode != 0:
            logger.error(
                f"Intensity computation in 3D failed for method {method} with exit code {exit_code.returncode}"
            )
        else:
