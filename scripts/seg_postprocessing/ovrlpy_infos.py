...
if compute_ovrlpy:
    ...
    integrity_matrix = np.load(
        pathlib.Path(sample_path, "vertical_doublets_ovrlpy_output.npz")
    )["integrity"]
    signal_matrix = np.load(pathlib.Path(sample_path, "vertical_doublets_ovrlpy_output.npz"))[
        "signal"
    ]
    for method in tqdm.tqdm(compute_ovrlpy):
        logger.info(f"Computing Ovrlpy statistics for {method}.")
        save_path = pathlib.Path(results_path, method, "Ovrlpy_stats")
        os.makedirs(save_path, exist_ok=True)
        sdata = SpatialData()
        tmp = read_zarr(
            pathlib.Path(results_path, method, "sdata.zarr"), selection=("shapes", "tables")
        )
        ...
        res.to_csv(pathlib.Path(save_path, "Ovrlpy_stats.csv"))
        plot_vsi_overview(
            ...
            png_path=pathlib.Path(save_path, "ovrlpy_overview_plot.png"),
        )
