...
handler.setFormatter(logging.Formatter(
    "%(asctime)s [%(levelname)s] Cell %(cell_id)s: %(message)s"
))
...
def determine_shapes_mapping(
    ...
    *,
    ...
    decisions_bottom = [vote_with_tiebreak(shape, top_trees)
                        for shape in bottom_list]
    # 2) top→bottom decisions
    decisions_top    = [vote_with_tiebreak(shape, bottom_trees)
                        for shape in top_list]
...
def triangulate(cell_geometry, z_offsets, scale_factors, cell_id):
    ...
    bottom_row = cell_geometry[cell_geometry.ZLevel == cell_geometry.ZLevel.min()].iloc[0]
    bottom_rings = extract_exterior_rings(bottom_row)

    # 1a) bottom dome caps
    for ring in bottom_rings:
        layers, dome_verts, dome_faces = preparation_dome(ring, -z_offsets, scale_factors)
    ...
    for idx in range(len(layers) - 1):
        ...
        for rel in rels:
            b_idxs = rel["bottom"] if isinstance(rel["bottom"], list) else [rel["bottom"]]
                    ...
                    if rel["type"] in ("many-to-one", "one-to-many"):
                        ...
                        def close(pts):
                            return np.vstack([pts, pts[0]]) \
                                if not np.allclose(pts[0], pts[-1]) else pts
...
save_path = f"/Users/jonasflor/Desktop/test_Cellpose3D_Paraview"
...
def main():
    layers = gpd.read_parquet("/Users/jonasflor/Downloads/cellpose2_mosaic_space.parquet")
    ...
    delayed = b.map(lambda cid: build_3D_cell(cid[0], cid[1], z_cap_height=14)).reduction(aggregate_dict, aggregate_list)
    delayed.compute()
