...
def merge_adatas(
    ...
    Each SpatialData object contains results of multiple segmentation approaches (for a single respective sample).
    This function extracts AnnData objects for a given segmentation method key, annotates them with metadata,
    optionally exports QC visualizations, and merges them into a single AnnData object. 
    No integration or batch effect correction is performed.

    Args:
        sdatas (List[Tuple[str, SpatialData]]): 
            List of tuples, each containing a unique sample name (str) and a SpatialData 
            object. Each SpatialData is expected to have an AnnData accessible via 
            `sdata[f"adata_{key}"]`.
        key (str): 
            Segmentation method key.
        sample_paths_file : str
            Dict that maps sample_name to Merscope output data path
        logger (logging.Logger, optional): 
            Logger instance for informational and warning messages. If None, logging 
            is disabled.
        do_qc (bool, optional): 
            If True, triggers QC plotting via `plot_qc`.
        save_path (str, optional): 
            File path or directory where QC plots will be saved if `do_qc` is True.

    Returns:
        AnnData: 
            A merged AnnData object containing concatenated cells from all input datasets.
    """
    adatas = []
...
def filter_spatial_outlier_cells(
    ...
    Args:
        adata: AnnData object with spatial coordinates in micron units in obsm["spatial"]
        data_dir: Directory containing sample subdirectories with CSV files
        sample_paths_file: Dict that maps sample_name to Merscope output data path
        save_path: Directory for saving plots
        logger: Optional logger object
        remove_outliers: If True (default), remove outlier cells from adata. 
                         If False, only mark outliers without filtering.
        
    Returns:
        AnnData
            Modified adata with "spatial_outlier" column added to obs.
    ...
    for idx, sample in enumerate(samples):
        ...
        if not isfile(csv_path):
            if logger:
                logger.info(f"[{sample}] Missing polygon file: {csv_path}. Draw outliers in 10X Explorer.")
        ...
        transform = np.loadtxt(join(sample_paths_file[sample], "images", "micron_to_mosaic_pixel_transform.csv")).reshape(3, 3)
        ...
        ax.scatter(coords[:, 0], coords[:, 1], c=np.where(outliers, "red", "grey"), s=0.4, alpha=0.75, edgecolors="none")
    ...
    fig.tight_layout()
    fig.savefig(join(save_path, "qc_spatial_outlier_cells.png"), dpi=150, bbox_inches="tight")
    plt.close()
...
def filter_low_quality_cells(
    ...
    Args:
        adata: AnnData object with spatial coordinates in micron units in obsm["spatial"]
        save_path: Directory for saving plots
        min_counts: Minimum number of counts per cell to retain
        min_genes: Minimum number of genes per cell to retain
        logger: Optional logger object
        remove_outliers: If True (default), remove outlier cells from adata. 
                         If False, only mark outliers without filtering.
    Returns:
...
def normalize_counts(
    ...
    *,
    ...
    Args:
        adata
            AnnData with raw integer counts in ``adata.X`` and a
            ``volume`` column in ``adata.obs``.
        save_path
            Directory for diagnostic plots.
        target_sum
            Target sum per cell after rescaling (default = 250 as in Allen et al.).
        logger
            Optional Python ``logging.Logger`` instance.

    Returns:
        AnnData
            ``adata`` with the layers above and outlier cells removed.
    """
    ...
    if not np.issubdtype(adata.X.dtype, np.integer):
        raise TypeError(
            f"adata.X must contain integer counts, found {adata.X.dtype}"
        )
    ...
    if logger:
        logger.info("Cells before/after outlier removal: " f"{adata.n_obs} → {mask.sum()}")
    
    for layer_name in adata.layers.keys():
        if sp.issparse(adata.layers[layer_name]) and not isinstance(adata.layers[layer_name], sp.csr_matrix):
            adata.layers[layer_name] = adata.layers[layer_name].tocsr()
    adata = adata[mask].copy()

    # diagnostic plot
    sns.histplot(row_sums, kde=False, edgecolor='none', 
                 color='steelblue')
    for p in (lo, hi):
    ...
    adata.layers["volume_norm"] = sp.csr_matrix(adata.layers["volume_norm"], dtype=np.float32)
    ...
    for ax, (layer, title) in zip(axes, layers):
        sns.histplot(adata.layers[layer].sum(1), bins=100, ax=ax, edgecolor='none', 
                 color='steelblue', alpha=1, legend=False, zorder=2)        
        ax.set_title(title)
        ax.grid(True, zorder=-1)
...
def dimensionality_reduction(adata: AnnData, save_path: str, logger=None) -> AnnData:
    ...
    Args:
        adata (anndata.AnnData): Annotated data matrix with a 'zscore' layer and relevant metadata in .obs.
    ...
    fig, axs = plt.subplots(3, 3, figsize=(18, 16), gridspec_kw={"wspace": 0.6, "hspace": 0.3})
    ...
    for row, config in enumerate(umap_configs):
        if logger:
            logger.info(f"Dimensionality reduction: UMAP with n_neighbors={config['n_neighbors']} and n_pcs={config['n_pcs']}")
...
def integration_harmony(
    ...
    Args:
        adata : AnnData
            AnnData data object. Must have 'zscore' layer and 'X_pca' obsm for PCA coordinates.
        batch_key : str
    ...
    Returns:
        AnnData
            Integrated data with new embeddings:
    ...
    with rc_context({'figure.figsize': (8,8)}):
        sc.pl.embedding(adata, 
                        ...
                        size=0.5, alpha=0.02, wspace=0.1, show=False,
                        projection='3d'
                       )
        plt.tight_layout()
        plt.savefig(join(save_path, "UMAP_integrated_harmony_3D.png"), dpi=200, bbox_inches='tight')
        plt.close()
...
def _plot_integration_comparison(adata: AnnData, save_path: str, umap_key: str) -> None:
    ...
    fig.text(0.72, 0.9, "Integrated (Harmony)", fontsize=24, fontweight='normal', ha='center')
    ...
    for i, (color_key, label) in enumerate(plot_configs):
        ...
        axes[i, 0].text(-0.1, 0.5, label, transform=axes[i, 0].transAxes,
                       fontsize=20, fontweight='normal', rotation=90, 
                       verticalalignment='center', horizontalalignment='center')
    
    plt.tight_layout()
    plt.savefig(join(save_path, "UMAP_integrated_harmony.png"), dpi=200, bbox_inches='tight')
    plt.close()
