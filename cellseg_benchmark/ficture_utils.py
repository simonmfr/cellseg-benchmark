...
def create_factor_level_image(data, factor, DAPI_shape) -> np.ndarray:
    ...
    image = np.clip(np.around(image * (np.finfo(np.float16).max.astype(np.uint16) - 4)), #ensures no overflow with np.float16
                    0, (np.finfo(np.float16).max.astype(np.uint16) - 4)).astype(np.uint16) #makes smaller file
