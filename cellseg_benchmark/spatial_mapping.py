...
label raster -> cleanup -> one polygon per connected component -> point-in-polygon.
"""
...
_K8 = np.array([[1, 1, 1], [1, 0, 1], [1, 1, 1]], int)  # 8-neighbourhood


def gridify(adata, label_key, coord_key="spatial_microns"):
    """Bin the cells of one sample back onto the raster they came from.

    Returns the (H, W) int32 label grid (-1 = background) and
    ...
    if np.any(cnt > 1):
        ...
        raise ValueError(
            f"Multiple cells map to pixel (row, col) {dup[0].tolist()}. "
            f"gridify expects a raster segmentation."
        )
...
def relabel_small_holes(grid, *, min_hole_area_um2, dx, dy, connectivity=1):
    """Fill enclosed inclusions up to min_hole_area_um2 with the surrounding label."""
...
def remove_small_islands(grid, *, min_island_area_um2, dx, dy, connectivity=1):
    """Reassign components up to min_island_area_um2 to their boundary majority."""
    g = np.asarray(grid).copy()
...
def polygons_per_component_exact(clean_grid, geo, value_map=None, connectivity=1):
    ...
    Returns dicts with keys poly, code, value (code through value_map) and comp_id.
    """
...
def _build_slide_index(label_polys):
    """Return prepared geometries, (label, index) metadata and a candidate lookup."""
    geoms, meta = [], []
...
def _map_points_for_slide(points_xy, prepared, meta, cand_fn, *, include_boundary=True):
    """Assign points of one slide to regions, returning labels and polygon indices."""
...
def map_points_to_regions_from_anndata(
    ...
    *,
    ...
    """Map every cell onto the region polygons of its slide.

    regions_by_slide is {slide: {label: [Polygon, ...]}}. Returns a dict keyed by
    slide with labels, poly_index, indices_and_labels and, if requested, df.
    """
...
def _extent_from_geo(grid, geo):
    """Return the imshow extent [left, right, bottom, top] in microns."""
    h, w = grid.shape
    x0, y0, dx, dy = (float(v) for v in geo)
...
def _overlay_polygons(regions, ax=None, *, highlight=None, highlight_edge_kw=None):
    """Draw region borders, highlighting an index or a (code, comp_id) selector."""
    if ax is None:
        _, ax = plt.subplots(figsize=(6, 5))
    ...
    if isinstance(highlight, int):
        selected = [highlight] if 0 <= highlight < len(regions) else []
    else:
        code, comp = highlight
        selected = [
            i
