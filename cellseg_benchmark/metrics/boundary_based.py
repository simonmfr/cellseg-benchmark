...
def count_assigned_transcripts(
    ...
    for boundaries_name, boundaries in available_boundaries.items():
        ...
        if boundaries is None or boundaries.empty or "geometry" not in boundaries.columns:
            print(f"  Skipping - invalid geometry")
            continue
        ...
        try:
            if "ZIndex" not in boundaries.columns:
                ...
                if len(unique_z) == 1:
                    ...
                    z_transcripts = transcripts_gdf[transcripts_gdf["global_z"] == unique_z[0]]
                    ...
                    for z_index in sorted(unique_z):
                        ...
                        z_transcripts = transcripts_gdf[transcripts_gdf["global_z"] == z_index]
                        
                        z_assigned, z_total = _process_boundaries(z_boundaries, z_transcripts)
            ...
            print(f"  {assigned}/{total} assigned ({results[dataset_name]['pct_assigned']}%) - {dimension}")
...
def _process_boundaries(boundaries_gdf, transcripts_subset):
    ...
    joined = gpd.sjoin(transcripts_subset, boundaries_unified, how="left", predicate="within")
